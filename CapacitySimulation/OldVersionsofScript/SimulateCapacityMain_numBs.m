...
%   mm_seperation_array = 75:25:350; % how many meters between consecutive base stations 
  ...
  hBs_index = floor(mod(NAI,length(numbs_array)*length(mm_hBs_array))/1/length(numbs_array)) + 1;
  seperation_index = mod(mod(NAI,length(numbs_array)*length(mm_hBs_array)), length(numbs_array)) + 1;
  
  mm_hBs = mm_hBs_array(hBs_index)
  num_Bs = numbs_array(seperation_index)
